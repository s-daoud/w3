...
def dept_staff_counts
  ...
  execute(<<-SQL)
    SELECT
      depts.name, SUM(COALESCE(staff.staff_count, 0))
    FROM
      depts
      LEFT JOIN (
        SELECT
          dept_id, COUNT(*) AS staff_count
        FROM
          teachers
        WHERE
          dept_id IS NOT NULL
