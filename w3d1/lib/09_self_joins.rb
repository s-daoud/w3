...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT
      stops.name, b.company, b.num
    FROM
      routes a
      JOIN routes b
      ON a.company = b.company AND a.num = b.num
      JOIN stops
      ON b.stop_id = stops.id
    WHERE
      a.stop_id = (
          SELECT
            stops.id
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT
      DISTINCT a.num, a.company, stops.name, b.num, b.company
    FROM
      routes a
      JOIN routes mid_a
      ON a.num = mid_a.num AND a.company = mid_a.company
      JOIN stops
      ON stops.id = mid_a.stop_id
      JOIN routes mid_b
      ON stops.id = mid_b.stop_id
      JOIN routes b
      ON b.num = mid_b.num AND b.company = mid_b.company
    WHERE
      a.stop_id = (
          SELECT
            id
